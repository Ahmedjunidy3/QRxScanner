    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // String result = finaluser.substring(0, finaluser.indexOf("@"));
        ...
        timer.schedule(new TimerTask() {
            @Override
            public void run() {
                startActivity(new Intent(additemActivity.this,FingerPrintActivity.class));
                finish();
            }
        }, 120000);
...
public  void additem(){
    ...
    if(!TextUtils.isEmpty(itemnameValue)&&!TextUtils.isEmpty(itempriceValue)){
        ...
        Items items = new Items(itemnameValue,itempriceValue,itembarcodeValue);
        databaseReference.child(resultemail).child("Items").child(itembarcodeValue).setValue(items);
        databaseReferencecat.child(resultemail).child("ItemByCategory").child(itembarcodeValue).setValue(items);
        itemname.setText("");
        itemprice.setText("");
        itembarcode.setText("");
